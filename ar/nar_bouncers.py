    ...
    def __init__(self, n, speed, color, nPixels):
        ...
        for i in range(self.locInt-self.n, self.locInt+self.n+1):
            if i == self.locInt:
                ...
                self.outArray[i,0] = color[0] * np.power(1.0 - (np.abs(self.locInt-i)/n) + 0.1,2)
                self.outArray[i,1] = color[1] * np.power(1.0 - (np.abs(self.locInt-i)/n) + 0.1,2)
                self.outArray[i,2] = color[2] * np.power(1.0 - (np.abs(self.locInt-i)/n) + 0.1,2)
    def update(self):
...
while True:
    for i in range(0,nBouncers):
        stripNum = np.mod(nBouncer, nStrips)
